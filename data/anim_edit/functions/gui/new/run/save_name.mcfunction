...
execute as @p[tag=anim_user] at @s if block ~ ~ ~ air run setblock ~ ~ ~ oak_sign{Text1:'["",{"text":"Model name: ","color":"aqua"},{"nbt":"name","storage":"anim:editor","color":"green"}]'}
...
execute as @p[tag=anim_user] at @s run data modify entity @e[type=armor_stand,tag=anim_sel,limit=1] ArmorItems[2].tag.anim.name set from storage anim:editor name
execute as @p[tag=anim_user] at @s run data modify entity @e[type=armor_stand,tag=anim_sel,limit=1] CustomName set from storage anim:editor name
execute as @p[tag=anim_user] at @s if score $_sign_placed _anim matches 1 run setblock ~ ~ ~ air
data modify storage anim:editor save_name set from storage anim:editor name
