...
execute if score $name_id _anim matches 1 run tellraw @p[tag=anim_user] ["",{"text":"Create a new animation","color":"gray","underlined":true},{"text":"\n\nGive the animation a name: ","color":"aqua"},{"text":"click here","underlined":true,"color":"gray","clickEvent":{"action":"suggest_command","value":"/data modify storage anim:anim name set value "}},{"text":"\n"}]
