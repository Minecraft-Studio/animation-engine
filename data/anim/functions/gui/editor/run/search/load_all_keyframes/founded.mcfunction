...
execute as @e[type=armor_stand,tag=anim_editor] if score $_search_p _anim = @s _anim_editor_row if score $_load_col _anim = @s _anim_editor_col run function anim:gui/editor/run/search/load_all_keyframes/founded2