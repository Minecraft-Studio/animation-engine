...
execute as @p[tag=anim_user] at @s if block ~ ~ ~ air run setblock ~ ~ ~ oak_sign{Text1:'{"nbt":"anim_part_cname","storage":"anim:editor"}'}
execute as @p[tag=anim_user] at @s run data modify entity @e[type=armor_stand,tag=anim_part_select,limit=1] CustomName set from block ~ ~ ~ Text1
execute as @p[tag=anim_user] at @s if score $_sign_placed _anim matches 1 run setblock ~ ~ ~ air

execute if score $_sign_placed _anim matches 1 run data remove storage anim:editor anim_part_cname
