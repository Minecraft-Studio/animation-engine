...
execute unless score $_tmp_clipboard_exists _anim matches 1 run tellraw @p[tag=anim_user] {"text":"Nothing to paste","color":"red"}
execute unless score $_paste_sel_exists _anim matches 1 run tellraw @p[tag=anim_user] {"text":"No keyframe is selected","color":"red"}
