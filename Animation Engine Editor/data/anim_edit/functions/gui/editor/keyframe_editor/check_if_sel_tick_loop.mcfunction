...
execute if score $_get_tick _anim = $_got_tick _anim if score $_get_part _anim = $_got_part _anim if score $_got_anim _anim = $_get_anim _anim store result score $_got_p_id _anim run data get storage anim:editor check_tick_sel[0].id
