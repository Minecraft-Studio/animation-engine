...
execute as @p[tag=anim_user] at @s unless entity @e[type=armor_stand,tag=anim_menu86,limit=1,tag=anim_sel_menu] if block ~ ~ ~ air run setblock ~ ~ ~ oak_sign{Text1:'{"score":{"name":"$page","objective":"_anim"},"color":"gray"}'}
execute as @p[tag=anim_user] at @s if entity @e[type=armor_stand,tag=anim_menu86,limit=1,tag=anim_sel_menu] if block ~ ~ ~ air run setblock ~ ~ ~ oak_sign{Text1:'{"score":{"name":"$page","objective":"_anim"},"color":"white"}'}
