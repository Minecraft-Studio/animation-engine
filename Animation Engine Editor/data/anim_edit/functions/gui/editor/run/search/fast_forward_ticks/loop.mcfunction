...
execute as @e[tag=anim_part] if score @s _anim_ida = @e[tag=anim_sel,limit=1] _anim_ida run function anim:keyframe_tick
# execute as @e[tag=anim_part] if score @s _anim_ida = @e[tag=anim_sel,limit=1] _anim_ida run tellraw @p [{"score": {"name": "$_fast_tick","objective": "_anim"},"color":"gold"},{"text":"/"},{"score": {"name": "@s","objective": "_anim_tick"},"color":"gold"},{"text":" / ","color":"red"},{"score": {"name": "$_key_col","objective": "_anim"},"color":"gold"}]
