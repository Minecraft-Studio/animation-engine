...
execute unless score @s _anim_id_player matches -2147483648..2147483647 run scoreboard players set @s _anim_sett_gmrle 1
...
execute unless score @s _anim_menu matches 3..7 if score $runtime_exists _anim matches 1 run scoreboard players set @s _anim_menu 3
function anim_edit:gui/main/load
# execute unless score $runtime_exists _anim matches 1 run tellraw @s ["",{"text":"Animation Engine Runtime is not installed! Cannot use the editor without it!","color":"red"}]